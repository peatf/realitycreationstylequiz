  ...
  return (
    <div className="bg-transparent p-6 rounded-3xl w-full">
      <div className="relative z-10">
        ...
        {/* Trait Description Cards - Horizontal Layout */}
        <div className="flex flex-wrap justify-between mb-8 gap-3">
          {dimensions.slice(0, 3).map((dimension) => (
            <div 
              key={dimension.id}
              className="p-4 w-[calc(33%-8px)] overflow-hidden relative inset-description-card"
              style={{
            ...
            >
              ...
              <h3 className="text-xs font-medium mb-1 relative z-10" style={{ color: "#2359FF" }}>
                {dimension.states[dimensionStates[dimension.id]]?.name || 'Balanced'}
              </h3>
              <p className="text-xs relative z-10" style={{ color: "#2359FF" }}>
                {dimension.states[dimensionStates[dimension.id]]?.description?.substring(0, 120)}...
              </p>
            </div>
          ))}
        </div>
