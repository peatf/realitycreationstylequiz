...
export const generateMasteryInsights = (
  ...
  // 3. Determine priority levels for each dimension
...
function getTransitionRecommendation(dimensionId, transitionKey, ambition, creativeState) {
  ...
  const transitionRecommendations = {
    ...
    // Similar expanded structures would be added for other dimensions
  };
...
function getTransitionRecommendation(dimensionId, transitionKey, ambition, creativeState) {
  ...
  const genericTransitions = {
    leftToBalanced: "Begin integrating elements from the opposite end of the spectrum, starting with low-risk situations.",
    balancedToLeft: "Focus more deliberately on developing the qualities from the left side of this spectrum.",
    balancedToRight: "Focus more deliberately on developing the qualities from the right side of this spectrum.",
    rightToBalanced: "While maintaining your strengths, begin selectively incorporating elements from the opposite approach.",
    leftToRight: "Start with small, gradual shifts toward the opposite approach, particularly in areas that feel comfortable.",
    rightToLeft: "Start with small, gradual shifts toward the opposite approach, particularly in areas that feel comfortable."
  };
  
  return genericTransitions[transitionKey] || "Focus on gradually moving toward your target state in this dimension.";
}
...
function calculateSynergyRelevance(fromPriority, toPriority, fromGrowth, toGrowth) {
  ...
  // Calculate base relevance from priorities
  const baseRelevance = (priorityWeights[fromPriority] + priorityWeights[toPriority]) / 2;
  
  // Adjust based on growth potential
  const growthFactor = (fromGrowth + toGrowth) / 2;
