 ...
 * 
 * @param {Object} answers - Key-value pairs of questionId: answerValue (1-5)
...
export const calculateResults = (answers, questions) => {
  ...
  Object.entries(questionsByDimension).forEach(([dimension, questionIds]) => {
    ...
    // Note: Slider values might need normalization if they're not on a 1-5 scale
    const normalizedAnswers = dimensionAnswers.map(value => {
      // Here we normalize the value to be between 1-5
      // Assuming slider values are 0-100, we normalize to 1-5
  ...
  const profileId = determineProfileId(dimensionStates);
